...
function shareText(score) {
    ...
    if (deviation_time > 23000) {
        return '炜叔，不觉得倒计时多了吗？' + ((deviation_time / 1000) - 20).toFixed(2) + "s";
    }
    SubmitResults();
    if (score <= 30) return '太虚了炜叔';
    if (score <= 60) return '炜叔就这';
    if (score <= 100) return '。。。。加油';
    if (score <= 150) return '不会吧，这么强';
    return '不亏是炜叔';
}
